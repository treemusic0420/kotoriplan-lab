...
export function PLVarianceDriversPage() {
  ...
  return <section className='rounded-xl bg-white p-6 shadow-sm'>
    ...
    <LearningNotes title='Variance Drivers' purpose='Identify which dimension values are driving favorable or unfavorable variance.' keyQuestion='Which dimension values are causing the biggest favorable or unfavorable variance?' whenToUse={['After finding a material variance in PL Variance.', 'When you need to know which product, customer, channel, or region caused the variance.']} howToRead={['Start with the biggest unfavorable drivers.', 'Then review favorable drivers.', 'Do not add values across independent dimensions.']} fpnaTips={['Driver analysis helps prioritize management actions.']} nextAction={['Prioritize the biggest unfavorable drivers.', 'Use the result to decide where management action is needed.']} />
