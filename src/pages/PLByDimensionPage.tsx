...
import { accountMeta, aggregatePlByDimension, orderedAccountKeys } from '../features/pl/api/plFactRepository'
...
export function PLByDimensionPage() {
  ...
  const accounts = orderedAccountKeys()
  return <section className='rounded-xl bg-white p-6 shadow-sm'><h2 className='text-lg font-medium'>PL by Dimension</h2><select className='mt-2 rounded border px-2 py-1' value={dimId} onChange={e => setDimId(e.target.value)}>{dims.map(d => <option key={d.id} value={d.id}>{d.name}</option>)}</select><div className='mt-4 overflow-x-auto'><table className='min-w-[800px] text-sm'><thead><tr><th>Account</th>{cols.map(([id, n]) => <th key={id}>{n}</th>)}<th>Total</th></tr></thead><tbody>{accounts.map(a => { const by = cols.map(([id]) => rows.filter((r: any) => r.analysis_dimension_value_id === id && r.pl_facts.account_key === a).reduce((s: number, r: any) => s + Number(r.pl_facts.amount), 0)); const total = by.reduce((s, n) => s + n, 0); const strong = accountMeta(a)?.isTotal || accountMeta(a)?.isProfitLine; return <tr key={a} className={strong ? 'font-semibold' : ''}><td>{accountMeta(a)?.accountName ?? a}</td>{by.map((n, i) => <td key={i} className={n < 0 ? 'text-rose-600' : ''}>{n.toLocaleString()}</td>)}<td className={total < 0 ? 'text-rose-600' : ''}>{total.toLocaleString()}</td></tr> })}</tbody></table></div></section>
}
