...
export function ScenarioListPage() {
  ...
  const filtered = useMemo(() => items.filter((it) => (`${it.name} ${it.productName || 'General'}`).toLowerCase().includes(q.toLowerCase()) && (status === 'all' || it.status === status) && (month === 'all' || it.targetYearMonth === month) && (product === 'all' || (it.productName || 'General') === product)).sort((a, b) => (b.createdAt.localeCompare(a.createdAt) || b.targetYearMonth.localeCompare(a.targetYearMonth))), [items, q, status, month, product])
